...
// Contexto regulatorio y comercial por país — alimenta los prompts de los agentes para
// que respondan con marco normativo, moneda y cultura de cierre del mercado real.
// Solo los 11 países LATAM con mayor probabilidad comercial. El resto cae a string vacío
// y los agentes responden con conocimiento genérico del modelo.
export const COUNTRY_CONTEXT: Record<string,string> = {
  "Chile":      "moneda CLP, regulador sanitario ISP, ambiental SEA/SMA, agrícola SAG, minero Sernageomin, factura electrónica SII obligatoria, plazo de pago típico 30-60 días, cierre rápido vs región pero exige fichas técnicas y certificaciones formales antes de aprobar proveedor.",
  "México":     "moneda MXN, regulador sanitario COFEPRIS, ambiental SEMARNAT, tributario SAT con CFDI obligatorio, T-MEC vigente con USA/Canadá, plazo de pago típico 60-90 días, decisión jerárquica con peso del dueño-fundador en empresas medianas, ciclo de venta más largo que Sudamérica, exige relación antes que contrato.",
  "Colombia":   "moneda COP, regulador sanitario INVIMA, ambiental ANLA, tributario DIAN con factura electrónica CUFE de transmisión inmediata, plazo de pago típico 60-90 días, comprador formal y orientado a procesos de licitación, exige RUT del proveedor y soporte de cumplimiento tributario.",
  "Perú":       "moneda PEN, regulador sanitario DIGEMID/DIGESA, ambiental MINAM/OEFA, tributario SUNAT con CPE obligatorio plazo 3 días, plazo de pago típico 60-90 días, comprador formal con detracciones del IGV, exige RUC vigente y validación de antecedentes del proveedor antes de contratar.",
  "Argentina":  "moneda ARS con alta inflación que tensiona ciclos de pago, regulador sanitario ANMAT, tributario ARCA (ex-AFIP) con CAE de factura electrónica, Mercosur vigente, plazo de pago típico 30-60 días pero con renegociación constante por inflación, comprador busca dolarizar el contrato o cláusulas de ajuste, decisor desconfiado por contexto macro.",
  "Ecuador":    "moneda USD (dolarización), regulador sanitario ARCSA, ambiental MAATE, tributario SRI con factura electrónica obligatoria, plazo de pago típico 60-90 días, comprador conservador y orientado a precio, mercado pequeño donde la referencia local pesa mucho.",
  "Uruguay":    "moneda UYU (a veces precios en USD), regulador sanitario MSP, ambiental MA, tributario DGI con e-factura CFE obligatoria, Mercosur vigente, plazo de pago típico 30-60 días, mercado chico y altamente formal, comprador exige cumplimiento estricto y proveedor con presencia regional.",
  "Bolivia":    "moneda BOB, regulador sanitario AGEMED, tributario SIN con factura electrónica en implementación gradual, plazo de pago típico 60-90 días, mercado con alta presencia estatal en sectores estratégicos (minería, hidrocarburos), comprador requiere documentación formal y NIT vigente.",
  "Paraguay":   "moneda PYG, regulador sanitario DINAVISA, tributario DNIT con factura electrónica SIFEN obligatoria, Mercosur vigente, plazo de pago típico 30-60 días, mercado pequeño con fuerte vinculación a Brasil y Argentina, comprador valora relación personal y referencias.",
  "Costa Rica": "moneda CRC (precios B2B frecuentemente en USD), regulador sanitario Ministerio de Salud, tributario Hacienda con factura electrónica obligatoria, plazo de pago típico 30-60 días, mercado formal con fuerte presencia de multinacionales en zonas francas (especialmente farma y dispositivos médicos), comprador exige certificaciones internacionales.",
  "Panamá":     "moneda USD oficial, regulador sanitario MINSA, tributario DGI con factura electrónica obligatoria desde 2022, plazo de pago típico 30-60 días, hub logístico regional con Zona Libre de Colón, comprador internacional y orientado a estándares globales, decisión más rápida que Sudamérica.",
};
  ...
  transaccional:{
    ...
    agentDefs:[
      {id:"compras_ag", name:"Jefe de Compras",  company:"CLIENTE",    side:"cliente",    icon:"🛒", accent:C.orange,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el Jefe de Compras de ${cl.name||"la empresa cliente"} en el sector ${m.name} de ${pais}. Un proveedor externo presenta "${cl.offerName||"una propuesta comercial"}" para reemplazar a tu proveedor actual. Situación actual: ${cl.situation||"evaluando cambio de proveedor"}. Presupuesto: ${cl.budget||"no especificado"}. Prioridades: precio final vs proveedor actual, condiciones de pago, garantía de suministro continuo, costo real del cambio. Responde en español (máx 140 palabras): objeciones principales y condiciones mínimas para avanzar. Sin protocolo, directo al punto.`},
      {id:"planta_ag",  name:"Jefe de Planta",   company:"CLIENTE",    side:"cliente",    icon:"⚙️", accent:C.orange,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el Jefe de Planta de ${cl.name||"la empresa cliente"} (${m.name}, ${pais}). Un proveedor propone "${cl.offerName||"una propuesta"}". Principal dolor: ${cl.pain||"optimización operacional"}. Prioridades: continuidad operacional, validación técnica, tiempo de transición, impacto en línea de producción. Responde en español (máx 140 palabras): exigencias técnicas concretas y riesgos operacionales que identificas.`},
      {id:"calidad_ag", name:"Jefe de Calidad",  company:"CLIENTE",    side:"cliente",    icon:"🔬", accent:C.green,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el Jefe de Calidad de ${cl.name||"la empresa cliente"} (${m.name}, ${pais}). Un proveedor presenta "${cl.offerName||"una propuesta"}". Palabras clave del sector: ${m.keywords?.join(", ")||"normativa sectorial"}. No apruebas nada sin documentación en regla. Responde en español (máx 140 palabras): documentación exigida y riesgos normativos que identificas.`},
      {id:"rival_ag",   name:"Competidor",        company:"COMPETENCIA", side:"competidor", icon:"⚔️", accent:C.red,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el ejecutivo comercial del principal competidor en el segmento ${m.name} en ${pais}. No representas ninguna empresa específica — nunca menciones nombres de empresas o marcas. Te enteras de que otro proveedor avanza en ${cl.name||"una empresa del sector"} con una propuesta de venta transaccional. Responde en español (máx 140 palabras): qué movimientos harías en los próximos 15 días, cómo te diferenciarías y qué argumento usarías con el decisor clave. Sin nombres de empresas.`},
      {id:"vendedor_ag",name:"Ejecutivo Comercial",company:"TU EMPRESA",  side:"vendedor",   icon:"🎯", accent:C.blueMain,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el ejecutivo comercial que presenta "${cl.offerName||"tu propuesta"}" — venta transaccional — a ${cl.name||"el cliente"} en ${m.name}, ${pais}. Ya escuchaste las objeciones de Compras, Planta y Calidad. El competidor ya se mueve en esta cuenta. Responde en español (máx 160 palabras): cómo manejas las 3 objeciones más duras, qué concesiones harías y cuáles no cederías bajo ningún concepto, y cómo llevas esto a un cierre concreto en las próximas 3 semanas.`},
    ],
  },
  servicio:{
    ...
    agentDefs:[
      {id:"ger_ag",     name:"Gerente General",   company:"CLIENTE",    side:"cliente",    icon:"🏢", accent:C.purple,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el Gerente General de ${cl.name||"la empresa cliente"} (${m.name}, ${pais}). Un proveedor externo ofrece "${cl.offerName||"un servicio especializado"}" — sin producto físico, prometiendo liberar capacidad interna. Situación actual: ${cl.situation||"evaluando proveedor externo"}. Prioridades: si el ROI es real y medible, si libera recursos para el negocio core. Responde en español (máx 140 palabras): qué argumentos te convencerían y qué condiciones pondría tu empresa para firmar.`},
      {id:"planta_ag",  name:"Jefe de Planta",    company:"CLIENTE",    side:"cliente",    icon:"⚙️", accent:C.orange,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el Jefe de Planta de ${cl.name||"la empresa cliente"} (${m.name}, ${pais}). Un proveedor ofrece "${cl.offerName||"un servicio"}". Dolor principal: ${cl.pain||"ineficiencia operacional"}. Podrías ser el campeón si el proveedor demuestra valor real. Responde en español (máx 140 palabras): qué problema real resolvería y qué reservas genuinas tienes hoy.`},
      {id:"legal_ag",   name:"Legal / Contratos", company:"CLIENTE",    side:"cliente",    icon:"⚖️", accent:C.red,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el responsable legal de ${cl.name||"la empresa cliente"} (${m.name}, ${pais}). Un proveedor propone "${cl.offerName||"un contrato de servicio"}". Debes revisar: SLA y penalidades, confidencialidad, condiciones de salida anticipada, responsabilidad legal por resultados. Responde en español (máx 130 palabras): cláusulas que exigirías y red flags contractuales que identificas.`},
      {id:"rival_ag",   name:"Competidor",         company:"COMPETENCIA", side:"competidor", icon:"⚔️", accent:C.red,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el ejecutivo comercial del principal competidor en servicios para el sector ${m.name} en ${pais}. Nunca menciones nombres de empresas o marcas. Te enteras de que otro proveedor avanza en ${cl.name||"una empresa"} con una propuesta de venta de servicio. Responde en español (máx 140 palabras): movimientos que harías en los próximos 15 días y cómo te diferenciarías. Sin nombres de empresas.`},
      {id:"vendedor_ag",name:"Ejecutivo Comercial", company:"TU EMPRESA",  side:"vendedor",   icon:"🎯", accent:C.blueMain,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el ejecutivo comercial que presenta "${cl.offerName||"tu propuesta de servicio"}" a ${cl.name||"el cliente"} en ${m.name}, ${pais}. El ROI es difícil de demostrar tangiblemente, el cliente cree que puede hacerlo internamente y Legal tiene reservas serias. Responde en español (máx 170 palabras): cómo construyes el caso de valor con datos concretos, qué concesiones harías en el contrato para destrabar la firma, y cómo demuestras el expertise diferencial antes de que firmen.`},
    ],
  },
  otro:{
    ...
    agentDefs:[
      {id:"gerencia_ag", name:"Gerencia",     company:"CLIENTE",    side:"cliente",    icon:"🏢", accent:"#636E72",
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el Gerente de ${cl.name||"la empresa cliente"} en ${m.name||"el mercado"}, ${pais}. Un proveedor presenta "${cl.offerName||"una propuesta"}". Modelo comercial: ${(cl as any).customModel||"no especificado"}. Contexto de mercado: ${m.customContext||m.context||"sector industrial"}. Situación: ${cl.situation||"evaluando proveedor"}. Responde en español (máx 140 palabras): qué argumentos te convencerían y qué condiciones exigirías para avanzar.`},
      {id:"compras_ag",  name:"Compras",      company:"CLIENTE",    side:"cliente",    icon:"🛒", accent:"#636E72",
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el responsable de Compras de ${cl.name||"la empresa cliente"} (${m.name||"mercado"}, ${pais}). Evalúas "${cl.offerName||"una propuesta"}". Modelo: ${(cl as any).customModel||"no especificado"}. Prioridades: precio, condiciones, garantías. Responde en español (máx 130 palabras): objeciones y condiciones mínimas para avanzar.`},
      {id:"tecnico_ag",  name:"Área Técnica", company:"CLIENTE",    side:"cliente",    icon:"⚙️", accent:"#636E72",
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el responsable técnico de ${cl.name||"la empresa cliente"} (${m.name||"mercado"}, ${pais}). Evalúas la viabilidad de "${cl.offerName||"una propuesta"}". Modelo: ${(cl as any).customModel||"no especificado"}. Responde en español (máx 130 palabras): exigencias técnicas y riesgos de implementación.`},
      {id:"rival_ag",    name:"Competidor",   company:"COMPETENCIA", side:"competidor", icon:"⚔️", accent:C.red,
       sys:(_m:any,cl:any,pais:string="Chile")=>`Eres el ejecutivo del principal competidor en el mercado de ${cl.name||"este cliente"}, ${pais}. Nunca menciones nombres de empresas. Te enteras de que otro proveedor avanza con "${cl.offerName||"una propuesta"}". Modelo: ${(cl as any).customModel||"no especificado"}. Responde en español (máx 130 palabras): movimientos que harías y cómo te diferenciarías.`},
      {id:"vendedor_ag", name:"Ejecutivo Comercial", company:"TU EMPRESA", side:"vendedor", icon:"🎯", accent:C.blueMain,
       sys:(m:any,cl:any,pais:string="Chile")=>`Eres el ejecutivo comercial que presenta "${cl.offerName||"tu propuesta"}" a ${cl.name||"el cliente"}, ${pais}. Modelo comercial propio: ${(cl as any).customModel||"no especificado"}. Contexto de mercado: ${m.customContext||m.context||"sector industrial"}. Dolor del cliente: ${cl.pain||"no especificado"}. Responde en español (máx 160 palabras): cómo manejas las objeciones principales, qué concesiones harías y cómo llevas esto a un cierre concreto.`},
    ],
  },
};
